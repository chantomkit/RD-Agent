...
# Feature handlers (base feature sets to build on)

Two ready-made handlers. The loop's base is a 20-feature subset (`ALPHA20`); when proposing, add expressions the base under-covers rather than duplicating it.


> ⚠️ Some catalog entries below reference `$vwap`, which is **absent from this dataset** (see fields.md). Those specific features resolve to NaN — do not adopt them.


## Alpha158 (158 features) — engineered K-line / rolling factors
...
## Alpha360 (360 features) — raw normalized price/volume history

Mechanical grid: roots `['CLOSE', 'HIGH', 'LOW', 'OPEN', 'VOLUME', 'VWAP']` × lag `k` in [0, 59], each normalized by the latest close/volume, e.g. `CLOSE59 = Ref($close, 59)/$close`, `VOLUME5 = Ref($volume, 5)/($volume+1e-12)`. Use when you want the model to see raw history.
